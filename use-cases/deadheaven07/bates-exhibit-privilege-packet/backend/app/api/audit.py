...
async def get_audit_trail(packet_id: UUID, session: AsyncSession = Depends(get_session)):
    ...
    return {
        ...
        "events": [
            serialize_event(
                event, document_names.get(str(event.document_id)) if event.document_id else None
            )
            for event in events
        ],
    }
...
async def get_document_audit(
    ...
    return {
        ...
        "total_events": len(events),
        "events": [serialize_event(event, document.original_filename) for event in events],
    }
