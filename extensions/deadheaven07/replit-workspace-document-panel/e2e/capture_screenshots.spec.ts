...
test('Capture High-Resolution Visual Screenshots of Modern UI/UX', async ({ page }) => {
  ...
  await page.route('https://api.superdocs.app/**', async (route) => {
    ...
    if (url.includes('/versions/v1') && method === 'GET') {
      return route.fulfill({
        ...
        body: JSON.stringify({
          version_id: 'v1',
          document_id: 'doc_demo_982',
          created_at: '2026-08-25T10:00:00Z',
          created_by: 'Initial Draft',
          change_summary: 'Initial workspace ingestion',
  ...
  await page.screenshot({ path: path.join(outDir, '01_dark_workspace_filetree.png') });
  console.log('Captured 01_dark_workspace_filetree.png');

  // 2. API Key Modal Pop
  await iframe.getByRole('button', { name: 'Set API Key' }).click();
  await page.waitForTimeout(300);
  await page.screenshot({ path: path.join(outDir, '02_api_key_modal_pop.png') });
  console.log('Captured 02_api_key_modal_pop.png');
  ...
  await page.waitForTimeout(300);
  await page.screenshot({ path: path.join(outDir, '03_cherry_pick_review.png') });
  console.log('Captured 03_cherry_pick_review.png');

  // 4. Side Drawer & Overview
  await iframe.getByLabel('Toggle Side Drawer').click();
  await page.waitForTimeout(300);
  await page.screenshot({ path: path.join(outDir, '04_side_drawer_overview.png') });
  console.log('Captured 04_side_drawer_overview.png');
  ...
  await page.waitForTimeout(300);
  await page.screenshot({ path: path.join(outDir, '05_template_gallery_variables.png') });
  console.log('Captured 05_template_gallery_variables.png');

  // 6. Version History
  await iframe.getByRole('tab', { name: 'History' }).click();
  await iframe.getByRole('button', { name: 'Preview' }).last().click();
  await page.waitForTimeout(300);
  await page.screenshot({ path: path.join(outDir, '06_version_history_rollback.png') });
  console.log('Captured 06_version_history_rollback.png');

  // 7. Light Theme Mode
  await iframe.getByLabel('Toggle Theme').click();
  await page.waitForTimeout(300);
  await page.screenshot({ path: path.join(outDir, '07_light_theme_mode.png') });
  console.log('Captured 07_light_theme_mode.png');
});
