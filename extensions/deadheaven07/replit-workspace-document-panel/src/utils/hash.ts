...
// unavailable (non-secure contexts, embedded webviews). Deterministic and
// interchangeable with the native implementation for change detection.
export function sha256Fallback(text: string): string {
  ...
  const padded = new Uint8Array((bytes.length + 8 + 64) & ~63);
  padded.set(bytes);
