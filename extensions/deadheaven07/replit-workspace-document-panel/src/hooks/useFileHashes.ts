...
import { usePersistedHashes } from './useStatePersistence';
...
export function useFileHashes(): FileHashState {
  const [persistedHashes, setPersistedHashes] = usePersistedHashes();
  ...
  const captureHashes = useCallback(async (files: Map<string, string>) => {
    ...
    await setPersistedHashes(hashes);
  }, [setPersistedHashes]);
